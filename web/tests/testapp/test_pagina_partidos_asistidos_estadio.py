...
def test_pagina_partidos_asistidos_estadio_sin_partidos(cliente, conexion_entorno_usuario):
...
def test_pagina_partidos_asistidos_estadio_sin_partidos_asistidos(cliente, conexion_entorno_usuario):
...
def test_pagina_partidos_asistidos_estadio_sin_estadios(cliente, conexion_entorno_usuario):
...
def test_pagina_partidos_asistidos_estadio_con_partido_asistido(cliente, conexion_entorno_usuario):
	...
	with cliente as cliente_abierto:
		...
		assert '<p class="valor-circulo-partidos-asistidos-estadio"><strong>1</strong></p>' in contenido
		assert '<p class="titulo-circulo-estadio">Metropolitano</p>'

@pytest.mark.parametrize(["cantidad_partidos", "cantidad_partidos_asistidos"],
	[(1,1),(2,1),(10,6),(7,3),(22,15)]
)
def test_pagina_partidos_asistidos_estadio_con_partidos_asistidos(cliente, conexion, password_hash, cantidad_partidos, cantidad_partidos_asistidos):

	conexion.c.execute("""INSERT INTO equipos (Equipo_Id) VALUES('atletico-madrid'),('rival')""")

	conexion.c.execute("""INSERT INTO estadios (Estadio_Id, Nombre, Capacidad, Codigo_Pais) VALUES('metropolitano', 'Metropolitano', 10000, 'es')""")
	...
	with cliente as cliente_abierto:
		...
		assert '<div class="tarjeta-partido-asistidos-estadio"' in contenido
		assert '<div class="info-partido-asistidos-estadio">' in contenido
		assert f'<p class="valor-circulo-partidos-asistidos-estadio"><strong>{cantidad_partidos_asistidos}</strong></p>' in contenido
		assert '<p class="titulo-circulo-estadio">Metropolitano</p>'

def test_pagina_partidos_asistidos_estadio_partido_asistido_estadisticas(cliente, conexion_entorno_usuario):
	...
	with cliente as cliente_abierto:
		...
		assert respuesta.status_code==200
		assert '<div class="circulo-estadisticas-partidos-asistidos-estadio">' in contenido
