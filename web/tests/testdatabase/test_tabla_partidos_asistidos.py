...
def test_obtener_partidos_no_asistidos_usuario_partido_asistido_otro_usuario(conexion_entorno):
	...
	conexion_entorno.insertarUsuario("otro", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")
...
def test_obtener_partidos_asistidos_usuario_estadio_no_existe_usuario(conexion):

	assert not conexion.obtenerPartidosAsistidosUsuarioEstadio("nacho", "atletico-madrid", "metropolitano")

def test_obtener_partidos_asistidos_usuario_estadio_no_existen_partidos(conexion):
...
def test_obtener_partidos_asistidos_usuario_estadio_no_existen_partidos_asistidos(conexion_entorno):
...
def test_obtener_estadio_partido_asistido_competicion_cantidad_no_existe_usuario(conexion):
 
 	assert not conexion.obtenerEstadiosPartidosAsistidosUsuarioCompeticionCantidad("nacho", 5)

def test_obtener_estadio_partido_asistido_competicion_cantidad_no_existen_partidos(conexion_entorno):
	...
	assert not conexion_entorno.obtenerEstadiosPartidosAsistidosUsuarioCompeticionCantidad("nacho", 5)

def test_obtener_estadio_partido_asistido_competicion_cantidad_no_existen_partidos_asistidos(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	assert not conexion_entorno.obtenerEstadiosPartidosAsistidosUsuarioCompeticionCantidad("nacho", 5)

def test_obtener_estadio_partido_asistido_competicion_cantidad_competicion_equipo_nula(conexion_entorno):
...
def test_obtener_estadio_partido_asistido_competicion_cantidad(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	conexion_entorno.insertarPartidoAsistido("20190622", "nacho", "comentario")

	estadios_competiciones=conexion_entorno.obtenerEstadiosPartidosAsistidosUsuarioCompeticionCantidad("nacho", 5)

	assert len(estadios_competiciones)==1
	assert estadios_competiciones[0][-2]==1
	assert estadios_competiciones[0][-1]==1

def test_obtener_estadio_partido_asistido_competicion_cantidad_otro_usuario(conexion_entorno):
	...
	assert not conexion_entorno.obtenerEstadiosPartidosAsistidosUsuarioCompeticionCantidad("otro", 5)

def test_obtener_estadio_partido_asistido_competicion_cantidad_visitado_y_total(conexion_entorno):
	...
	for numero in range(1,10):
		...
		conexion_entorno.c.execute(f"""INSERT INTO equipos (Equipo_Id, Competicion, Codigo_Competicion) VALUES('equipo{numero}', 'Primera', 'primera')""")
	...
	assert len(estadios_competiciones)==1
	assert estadios_competiciones[0][-2]==1
	assert estadios_competiciones[0][-1]==10

def test_obtener_estadio_partido_asistido_competicion_cantidad_varias_competiciones(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	for numero in range(1,21):

		conexion_entorno.c.execute(f"""INSERT INTO competiciones VALUES('codigo_competicion{numero}', 'Primera{numero}', 'primera-division-ea{numero}', 'es')""")

		conexion_entorno.c.execute(f"""INSERT INTO equipos (Equipo_Id, Competicion, Codigo_Competicion) VALUES('equipo{numero}', 'competicion{numero}', 'codigo_competicion{numero}')""")
	...
	assert len(partidos_asistidos)==20
	assert len(estadios_competiciones)==5

	for estadio_competicion in estadios_competiciones:
		assert estadio_competicion[-2]==1
		assert estadio_competicion[-1]==1

def test_obtener_estadio_partido_asistido_competicion_cantidad_visitados_y_total(conexion_entorno):
	...
	for numero in range(1,6):
		...
		conexion_entorno.c.execute(f"""INSERT INTO equipos (Equipo_Id, Competicion, Codigo_Competicion) VALUES('equipo{numero+10}', 'competicion{numero}', 'codigo_competicion{numero}')""")

		conexion_entorno.c.execute(f"""INSERT INTO estadios (Estadio_Id, Capacidad) VALUES('estadio{numero+10}', 10000)""")

		conexion_entorno.c.execute(f"""INSERT INTO equipo_estadio VALUES('equipo{numero+10}', 'estadio{numero+10}')""")
	...
	assert len(partidos_asistidos)==5
	assert len(estadios_competiciones)==5

	for estadio_competicion in estadios_competiciones:
		assert estadio_competicion[-2]==1
		assert estadio_competicion[-1]==2

def test_obtener_estadio_competicion_partido_asistido_no_existe_usuario(conexion):
 
 	assert not conexion.obtenerEstadiosCompeticionPartidosAsistidosUsuario("nacho", "primera", 5)

def test_obtener_estadio_competicion_partido_asistido_no_existen_estadios(conexion_entorno):
	...
	assert not conexion_entorno.obtenerEstadiosCompeticionPartidosAsistidosUsuario("nacho", "primera", 5)

def test_obtener_estadio_competicion_partido_asistido_no_existe_competicion(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	assert not conexion_entorno.obtenerEstadiosCompeticionPartidosAsistidosUsuario("nacho", "no-existo", 5)

def test_obtener_estadio_competicion_partido_asistido_no_existen_partidos(conexion_entorno):
	...
	estadios=conexion_entorno.obtenerEstadiosCompeticionPartidosAsistidosUsuario("nacho", "primera", 5)

	assert len(estadios)==1
	assert estadios[0][-1]==0

def test_obtener_estadio_competicion_partido_asistido_no_existen_partidos_asistidos(conexion_entorno):
...
def test_obtener_estadio_competicion_partido_asistido_competicion_equipo_nula(conexion_entorno):

	conexion_entorno.c.execute("UPDATE equipos SET Codigo_Competicion=NULL")
...
def test_obtener_estadio_competicion_partido_asistido(conexion_entorno):
...
def test_obtener_estadio_competicion_partido_asistido_otro_usuario(conexion_entorno):
	...
	estadios=conexion_entorno.obtenerEstadiosCompeticionPartidosAsistidosUsuario("otro", "primera", 5)

	assert len(estadios)==1
	assert estadios[0][-1]==0

def test_obtener_estadio_competicion_partido_asistido_varios_estadios(conexion_entorno):
	...
	for estadio in estadios[1:]:
		assert estadio[-1]==0

def test_obtener_ciudades_estadio_partido_asistido_cantidad_no_existe_usuario(conexion):

	assert not conexion.obtenerCiudadesEstadiosPartidosAsistidosUsuarioCantidad("nacho", 5)

def test_obtener_ciudades_estadio_partido_asistido_cantidad_no_existen_partidos(conexion_entorno):
	...
	assert not conexion_entorno.obtenerCiudadesEstadiosPartidosAsistidosUsuarioCantidad("nacho", 5)

def test_obtener_ciudades_estadio_partido_asistido_cantidad_no_existen_partidos_asistidos(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	assert not conexion_entorno.obtenerCiudadesEstadiosPartidosAsistidosUsuarioCantidad("nacho", 5)

def test_obtener_ciudades_estadio_partido_asistido_cantidad_ciudad_nula(conexion_entorno):
...
def test_obtener_ciudades_estadio_partido_asistido_cantidad(conexion_entorno):
	...
	ciudades_estadios=conexion_entorno.obtenerCiudadesEstadiosPartidosAsistidosUsuarioCantidad("nacho", 5)

	assert len(ciudades_estadios)==1
	assert ciudades_estadios[0][-1]==1

def test_obtener_ciudades_estadio_partido_asistido_cantidad_otro_usuario(conexion_entorno):
...
def test_obtener_ciudades_estadio_partido_asistido_cantidad_varios(conexion_entorno):
	...
	for numero in range(1,21):
		...
		conexion_entorno.c.execute(f"""INSERT INTO estadios (Estadio_Id, Capacidad, Pais, Ciudad) VALUES('estadio{numero}', 10000, 'pais', 'ciudad{numero}')""")
	...
	assert len(partidos_asistidos)==20
	assert len(ciudades_estadios)==5

	for ciudad in ciudades_estadios:

		assert ciudad[-1]==1

def test_obtener_ciudades_estadio_partido_asistido_cantidad_mismo_varias_veces(conexion_entorno):
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_no_existe_usuario(conexion):

	assert not conexion.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("nacho", "es", 5)

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_no_existen_partidos(conexion_entorno):
	...
	assert not conexion_entorno.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("nacho", "es", 5)

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_no_existen_partidos_asistidos(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	assert not conexion_entorno.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("nacho", "es", 5)

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_ciudad_nula(conexion_entorno):
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_otro_pais(conexion_entorno):

	conexion_entorno.insertarUsuario("nacho", "micorreo@correo.es", "1234", "nacho", "dorado", "1998-02-16", 103, "atletico-madrid")

	conexion_entorno.insertarPartidoAsistido("20190622", "nacho", "comentario")

	assert not conexion_entorno.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("nacho", "se", 5)

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_codigo_pais_estadio_equipo_nulos(conexion_entorno):
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_codigo_pais_estadio_nulo(conexion_entorno):
	...
	ciudades_estadios=conexion_entorno.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("nacho", "es", 5)

	assert len(ciudades_estadios)==1
	assert ciudades_estadios[0][-1]==1

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_codigo_pais_equipo_nulo(conexion_entorno):
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad(conexion_entorno):
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_otro_usuario(conexion_entorno):
	...
	assert not conexion_entorno.obtenerCiudadesEstadiosPaisPartidosAsistidosUsuarioCantidad("otro", "es", 5)

def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_varios(conexion_entorno):
	...
	for numero in range(1,21):
		...
		conexion_entorno.c.execute(f"""INSERT INTO estadios (Estadio_Id, Capacidad, Codigo_Pais, Pais, Ciudad) VALUES('estadio{numero}', 10000, 'es', 'pais', 'ciudad{numero}')""")
...
def test_obtener_ciudades_estadios_pais_partido_asistido_cantidad_varias_veces(conexion_entorno):
